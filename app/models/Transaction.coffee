module.exports = (db) ->
	...
	class Transaction
		...
		@getUnmatched: (done) ->
			db.query 'SELECT id, amount, description, date, balance, consider_matched, md5sum FROM transactions WHERE consider_matched IS NULL AND id NOT IN (SELECT transaction_id FROM future_transactions WHERE transaction_id >= 1) ORDER BY date DESC', [],
				(err, res) ->
					if err
						done err.toString()
					else
